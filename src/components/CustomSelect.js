  ...
  return <div className="dropdown-container">
    ...
    <div className="dropdown-input">
      ...
      <div className='dropdown-values'>
        {value.length ? value.map(v => <div key={v} className='dropdown-value'> {options[v].label} <span className='dropdown-remove' onClick={() => removeValue(v)}>X</span></div>) : <div onClick={() => setIsActive(!isActive)} className='dropdown-placeholder'> Please select a currency </div> }
      </div>
    </div>
